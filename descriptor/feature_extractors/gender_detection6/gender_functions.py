...
cur_dir = os.path.split(os.path.abspath(__file__))[0] + '/gender_model_rothe'
...
def create_transformer(network, mean_values):
    """
    Create transformer associated to the network
    """
    gender_transformer = caffe.io.Transformer({'data': network.blobs['data'].data.shape})

    gender_transformer.set_transpose('data', (2, 0, 1))     # Move image channels to outermost dimension
    gender_transformer.set_mean('data', mean_values)        # Subtract the dataset-mean value in each channel
    gender_transformer.set_raw_scale('data', 255)           # Rescale from [0, 1] to [0, 255]
    gender_transformer.set_channel_swap('data', (2, 1, 0))  # Swap channels from RGB to BGR
    return gender_transformer
...
def create_network(mean,model):
    ...
    net_model_file = os.path.join(cur_dir,'gender.prototxt')
