...
class CnnHeadPoseEstimator:
    ...
    def _init_yaw_(self, YawFilePath):
        ...
        def model(data):
            ...
            # Convolution Layer 4
            conv4 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv3, self.hy_conv4_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hy_conv4_biases))
            if(DEBUG == True): print("SHAPE conv4: " + str(conv4.get_shape()))

            # Fully connected layer 5
            dense1 = tf.reshape(conv4, [-1, self.hy_dense1_weights.get_shape().as_list()[0]]) # Reshape conv3
            if(DEBUG == True): print("SHAPE dense1: " + str(dense1.get_shape()))
            dense1 = tf.nn.relu(tf.matmul(dense1, self.hy_dense1_weights) + self.hy_dense1_biases) # Relu

            #Fully connected layer 6
            dense2 = tf.nn.relu(tf.matmul(dense1, self.hy_dense2_weights) + self.hy_dense2_biases) # Relu
    ...
    def _init_pitch_(self, PitchFilePath):
        ...
        def model(data, _dropout=1.0):
            ...
            # Convolution Layer 1
            conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(X, self.hp_conv1_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hp_conv1_biases))
            if(DEBUG == True): print("SHAPE conv1: " + str(conv1.get_shape()))
            # Max Pooling (down-sampling)
            pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            if(DEBUG == True): print("SHAPE pool1: " + str(pool1.get_shape()))
            ...
            # Convolution Layer 2
            conv2 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm1, self.hp_conv2_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hp_conv2_biases))
            if(DEBUG == True): print("SHAPE conv2: " + str(conv2.get_shape()))
            # Max Pooling (down-sampling)
            pool2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            if(DEBUG == True): print("SHAPE pool2: " + str(pool2.get_shape()))
            ...
            # Convolution Layer 3
            conv3 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm2, self.hp_conv3_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hp_conv3_biases))
            if(DEBUG == True): print("SHAPE conv3: " + str(conv3.get_shape()))
            # Max Pooling (down-sampling)
            #pool3 = tf.nn.max_pool(conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            #if(DEBUG == True): print("SHAPE pool3: " + str(pool3.get_shape()))
            ...
            # Convolution Layer 4   
            conv4 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv3, self.hp_conv4_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hp_conv4_biases))
            print("SHAPE conv4: " + str(conv4.get_shape())) 

            # Fully connected layer 4
            dense1 = tf.reshape(conv4, [-1, self.hp_dense1_weights.get_shape().as_list()[0]]) # Reshape conv3
            if(DEBUG == True): print("SHAPE dense1: " + str(dense1.get_shape()))
            dense1 = tf.nn.relu(tf.matmul(dense1, self.hp_dense1_weights) + self.hp_dense1_biases) # Relu

            #Fully connected layer 5
            dense2 = tf.nn.relu(tf.matmul(dense1, self.hp_dense2_weights) + self.hp_dense2_biases) # Relu
            if(DEBUG == True): print("SHAPE dense2: " + str(dense2.get_shape()))
    ...
    def return_yaw_probability(self, image):
         ...
         if(h == w and h>39):
             ...
             raise ValueError('CnnHeadPoseEstimator: the image given as input is not squared or it is smaller than 40px.')
