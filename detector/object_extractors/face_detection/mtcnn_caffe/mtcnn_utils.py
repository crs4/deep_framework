...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    if numbox > 0:
        ...
        temp_ind= pad(total_boxes, w, h)
        [dy, edy, dx, edx, y, ey, x, ex, tmpw, tmph] = [list(map(round_to_int, ind_list)) for ind_list in temp_ind]
